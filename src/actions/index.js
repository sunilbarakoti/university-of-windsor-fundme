...
export const createCampaign = formValues => async (dispatch, getState) => {
  ...
  const response = await axios({
    method: "post", url: baseURL + '/api/campaign',
    headers: {
...
export const fetchCampaigns = () => async dispatch => {
  await axios({
    method: "post", url: baseURL + '/api/campaign/active',
    headers: {
      "Content-Type": "application/json",
      'Authorization': 'Bearer ' + localStorage.getItem('access_token')
    },
...
export const fetchAwaitingCampaigns = () => async dispatch => {
  await axios({
    method: "post", url: baseURL + '/api/campaign/awaiting',
    headers: {
      "Content-Type": "application/json",
      'Authorization': 'Bearer ' + localStorage.getItem('access_token')
    },
...
export const fetchCampaign = (id) => async dispatch => {
  const response = await axios({
    method: "post", url: baseURL + '/api/campaign/list',
    headers: {
      "Content-Type": "application/json",
      'Authorization': 'Bearer ' + localStorage.getItem('access_token')
    },
...
export const editCampaign = (currentData, formValues) => async dispatch => {
  const response = await axios({
    method: "put", url: baseURL + '/api/campaign',
    headers: {
      "Content-Type": 'multipart/form-data',
      'Authorization': 'Bearer ' + localStorage.getItem('access_token')
    },
  ...
  dispatch({ type: EDIT_CAMPAIGN, payload: currentData });
  history.push('/dashboard');
}

export const approveCampaign = (formValues) => async dispatch => {
  const response = await axios({
    method: "patch", url: baseURL + '/api/mgo/action',
    headers: {
      "Content-Type": 'application/json',
      'Authorization': 'Bearer ' + localStorage.getItem('access_token')
    },
  ...
  debugger;
  dispatch({ type: APPROVE_CAMPAIGN, payload: formValues });
}

export const deleteCampaign = (id) => async dispatch => {

  const response = await axios({
    method: "delete", url: baseURL + `/api/campaign?id=${id}`,
    headers: {
      "Content-Type": "application/json",
      'Authorization': 'Bearer ' + localStorage.getItem('access_token')
    },
